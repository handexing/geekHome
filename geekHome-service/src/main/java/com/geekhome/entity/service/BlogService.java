...
public class BlogService {
	...
	@SuppressWarnings("unchecked")
	public Page<Blog> getBlogList(Integer page, Integer rows, Long systemTypeId) {
		...
		sql.append(
				...
				.append(" FROM blog AS o LEFT JOIN BLOG_TYPE AS l ON o.TYPE_ID = l.ID LEFT JOIN BLOG_TYPE AS t ON o.SYSTEM_TYPE_ID = t.ID LEFT JOIN user AS u ON u.ID = l.USER_ID");

		if (systemTypeId != 0) {
			sql.append(" WHERE o.SYSTEM_TYPE_ID=" + systemTypeId);
			total = blogDao.getBoleBySystemTypeIdCnt(systemTypeId);
		} else {
			total = blogDao.getBoleCnt();
		}
	...
	@SuppressWarnings("unchecked")
	public Page<Blog> getHotBlogList(Integer page, Integer rows) {
		int firstRecord = PageUtil.calcPage(page) * rows;
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT o.ID id,l.ID typeId,t.ID systemTypeId,l.`NAME` typeName,t.`NAME` systemTypeName,o.TITLE title,o.SUBTITLE subtitle,o.CONTENT content,o.BANNER_IMG bannerImg,o.`STATUS` status,o.COLLECT_COUNT collectCount,o.BROWSE_COUNT browseCount,o.CREATE_TIME createTime,")
				.append("o.UPDATE_TIME updateTime,(SELECT COUNT(1) FROM COMMENT WHERE THEME_ID = o.ID AND TYPE=3) as commentCnt,u.USER_NAME userName,u.HEAD_IMG_URL headImgUrl,l.USER_ID userId")
				.append(" FROM blog AS o LEFT JOIN BLOG_TYPE AS l ON o.TYPE_ID = l.ID LEFT JOIN BLOG_TYPE AS t ON o.SYSTEM_TYPE_ID = t.ID LEFT JOIN user AS u ON u.ID = l.USER_ID ORDER BY o.BROWSE_COUNT DESC");
		List<Blog> list = entityManager.createNativeQuery(sql.toString(), "getAllBlogList").setFirstResult(firstRecord)
				.setMaxResults(rows).getResultList();
	...
	@SuppressWarnings("unchecked")
	public Page<Blog> getHotBlogUserList(Integer page, Integer rows) {
		int firstRecord = PageUtil.calcPage(page) * rows;
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT u.USER_NAME userName,u.HEAD_IMG_URL headImgUrl,l.USER_ID userId,(SELECT COUNT(1) FROM COMMENT WHERE THEME_ID = o.ID AND TYPE=3) as commentCnt")
				.append(" FROM blog AS o LEFT JOIN BLOG_TYPE AS l ON o.TYPE_ID = l.ID LEFT JOIN BLOG_TYPE AS t ON o.SYSTEM_TYPE_ID = t.ID")
				.append(" LEFT JOIN user AS u ON u.ID = l.USER_ID GROUP BY l.USER_ID ORDER BY commentCnt DESC,o.BROWSE_COUNT DESC,o.COLLECT_COUNT DESC");
		List<Blog> list = entityManager.createNativeQuery(sql.toString(), "getHotBlogUserList")
