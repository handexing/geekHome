...
public class AdminShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		Admin userInfo = adminDao.findAdminByUserName(username);
		if (userInfo == null) {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) throws AuthenticationException {
		...
		if (userInfo.getIsSystem() == 1) {
			menus = menuDao.getAllMenuCode();
		} else {
			menus = menuDao.findMenuCodeByUserId(userInfo.getId());
		}
	...
	public Set<String> getStringCode(Set<Menu> code) {
		...
		if (code != null) {
			for (Menu m : code) {
				stringPermissions.add(m.getCode());
			}
		}
